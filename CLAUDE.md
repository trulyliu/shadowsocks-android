...
- **`applicationId`** defaults to `com.github.skcoswodahs.tv` (`shadowsocks` reversed). The `namespace` (R-class package) stays `com.github.shadowsocks.tv` — AGP allows the two to differ, so all Kotlin source under `com.github.shadowsocks.tv.*` is unchanged. **Override per build** with `-PCNTV_APPLICATION_ID=com.example.foo.tv` (gradle property) or `CNTV_APPLICATION_ID=...` (env var) when a vendor blocklists the default; `build-cntv-release.sh` forwards either source automatically.
